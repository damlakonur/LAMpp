...
@dataclass()
class CafcaDataConfig(DataConfig):
    _target: str = "vhap.data.cafca_dataset.CafcaDataset"
...
@dataclass()
class CafcaLossWeightConfig(LossWeightConfig):
...
@dataclass()
class CafcaStageRgbSequentialTrackingConfig(StageRgbSequentialTrackingConfig):
...
@dataclass()
class CafcaStageRgbGlobalTrackingConfig(StageRgbGlobalTrackingConfig):
    align_texture_except: tuple[str, ...] = ("boundary",)
    align_boundary_except: tuple[str, ...] = ("boundary",)
    """Due to the limited flexibility in the lower neck region of FLAME, we relax the 
    alignment constraints for better alignment in the face region.
...
@dataclass()
class CafcaPipelineConfig(PipelineConfig):
    rgb_sequential_tracking: CafcaStageRgbSequentialTrackingConfig
    rgb_global_tracking: CafcaStageRgbGlobalTrackingConfig

@dataclass()
class CafcaTrackingConfig(BaseTrackingConfig):
    data: CafcaDataConfig
    w: CafcaLossWeightConfig
    pipeline: CafcaPipelineConfig
...
if __name__ == "__main__":
    config = tyro.cli(CafcaTrackingConfig)
